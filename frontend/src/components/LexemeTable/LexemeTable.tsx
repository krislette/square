...
interface LexemeTableProps {
  lexerResponse: LexerResponse | null;
}
...
export default function LexemeTable({ lexerResponse }: LexemeTableProps) {
  ...
  if (lexerResponse.status === "error") {
    return (
      <div id="lexeme" className="error-container">
        <div className="error-message">
          <h3>Lexical Error</h3>
          <p>{lexerResponse.error?.message}</p>
        </div>
      </div>
    );
  }

  const filteredLexemes = lexerResponse.tokens?.filter((lexeme) => {
    const matchesSearch = lexeme.lexeme
      .toLowerCase()
              ...
              {filteredLexemes.map((lexeme, index) => (
                <tr key={index}>
                  ...
                  <td className="td-token">
                    <span
                      className={`token-tag token-${lexeme.token.toLowerCase()}`}
                    >
                      {lexeme.token}
                    </span>
                  </td>
